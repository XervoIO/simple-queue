...
Client.prototype.watchQueue = function (subscription) {
  ...
  var poll = function () {
    ...
    request({
      ...
      if (err) self.debugLog('An error occurred while request queue message: ' + err)
      // There is no message.
      else if (response.statusCode === 204) self.debugLog('No message available for queue: ' + subscription.queue)
