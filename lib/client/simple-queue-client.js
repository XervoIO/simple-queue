...
Client.prototype.watchQueue = function(subscription) {
  ...
  var poll = function() {
    ...
    request({
      ...
      if(err) {
        self.emit('error', new Error('Failed to request queue message: ' + err.code));
      ...
      // A message is available.
        ...
        try {
          ...
          self.emit('error', new Error('Failed to JSON-parse incoming message.'));
...
Client.prototype._enqueue = function(queue, message, callback) {
  ...
  var enqueue = function() {
    ...
    request({
      ...
      if(err) {
        self.emit('error', new Error('Failed to queue message: ' + err.code));
